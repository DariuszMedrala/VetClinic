<?php
$eventColor = static fn (string $status): string => 'event--teal';
...
<?php foreach ($availability as $weekday => $win):
    $col = (int) $weekday + 1;
    [$sh, $sm] = array_map('intval', explode(':', $win['start']));
    [$eh, $em] = array_map('intval', explode(':', $win['end']));
    if ($sh > 17) { continue; }
    $rowStart = $sh - 7;
    $topMin = $sm;
    if ($rowStart < 1) { $rowStart = 1; $topMin = 0; }
    $maxSpan = (11 - $rowStart) * 60;
    $span = min($topMin + (($eh * 60 + $em) - ($sh * 60 + $sm)), $maxSpan);
    $heightMin = max(20, $span - $topMin);
    $top = round($topMin / 60 * 64, 1);
    $height = round($heightMin / 60 * 64, 1);
?>
          <div class="cal-avail" style="grid-column:<?= $col ?>;grid-row:<?= $rowStart ?>;align-self:start;margin-top:<?= $top ?>px;height:<?= $height ?>px"></div>
<?php endforeach; ?>
...
<?php endif; ?>
      ...
      <script src="<?= e(asset('/assets/js/calendar.js')) ?>"></script>
